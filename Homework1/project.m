...
%  cam.m : physical pixel resolution aka magnification factor [pixels/meter]  (2x1 vector)
%  cam.p : camera principal point [in pixels]  (2x1 vector)
%
...
% drop points which have non-positive z coordinates
% since we can't see them in the camera
posz = find(Xcam(3,:) > 0);
Xcam = Xcam(:,posz);
...
% scale by focal length, pixel magnification and add in camera principal point offset.
x(1,:) = x(1,:)*cam.f*cam.m(1) + cam.c(1);
x(2,:) = x(2,:)*cam.f*cam.m(2) + cam.c(2);
