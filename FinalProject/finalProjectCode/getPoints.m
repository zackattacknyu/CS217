...
%   This was used to select the points from
%           the topographic map
...
%these are the points selected on the topographic map
xL = [698.491967871486
          676.002008032129
          767.568273092369
          590.861445783133
...
%used to find the scale of the topographic map image
...
%these are the points computed above
...
% This section of code computes the final 
%   3D coordinates for our calibration points
...
% gets points in the right photo where those landmarks occur
...
%calibrates the right camera
points2D = [xP';yP'];
points3D = xyzPoints';

%attempt at calibrating camera
%       calibrate function is one I made in Homework 1
camR = calibrate(points3D,points2D);

%%

%gets points for left camera shot and then
%       calibrates the points
...
%saves the different groups of points
...
%concatenates the groups of points
xTR = [xTR1' xTR2' xTR3'];
yTR = [yTR1' yTR2' yTR3'];
...
%puts the manually picked points into the group
%       to put through triangulate function
left2Dpts = [xTL;yTL];
right2Dpts = [xTR;yTR];

%%
%to run after using SIFT to find the 2D points
% puts those points into group to put through 
...
%   function is the one I made for Homework 1
allPts3D = triangulate(left2Dpts,right2Dpts,camL,camR);

%%

%this made the z=0 plane to represent the lake
...
%plots all the 3D points
%   used to show the reconstruction from SIFT
figure
...
%plots all the 3D points
%   used to show the reconstruction from manual point selection
figure
